...
export default class FullCalendarJs extends NavigationMixin(LightningElement) {
    ...
    @track receiptStepCompleted = false; // added by trupti
                        ...
                        fetchServiceAtFacility({ serviceAtFacility: this.servicesAtFacilityId })
                            .then(data => {
                                if (data) {
                                    ...
                                    if (data.Name.includes('Picnic Shelter'))
                                        this.isPicnicShelter = true;
                                }
                            })
                            .catch((error) => {
    ...
    */
    ...
    handleCancel() {
        this.attendeeList = [];
        var newItem = [{ id: 0, emailclass: 'validateTwo hideemail', isRadioChecked: false, attendeefirstname: '', attendeelastname: '', attendeegender: '', attendeeage: '', attendeeemail: '', attendeephone: '' }];
    ...
    */
    ...
    addRow() {
        ...
        if (this.attendeeList.length < (15 - (this.availableslots[this.splitstartDate] || 0))) {
            ...
            ++this.keyIndex;
            var newItem = [{ id: this.keyIndex, emailclass: 'validateTwo hideemail', isRadioChecked: false, attendeefirstname: '', attendeelastname: '', attendeegender: '', attendeeage: '', attendeeemail: '', attendeephone: '' }];
            this.attendeeList = this.attendeeList.concat(newItem);
            ...
            fetchUserInfo()
                .then(data => {
                    if (data) {
                        ...
                        this.eventDetails = {enddate:this.endDate, endtime: this.endTime, startdate: this.startDate, enddate: this.endDate, starttime : this.startTime, activitytype: this.activityType, recreationalid: this.facilityId };
                        ...
                        createPaymentTransaction({ paymentResponseJSON1: JSON.stringify(this.paymentDetails) })
                            .then((result) => {
                                ...
                                this.receiptStepCompleted = true;
    ...
    */
    ...
    pathClickHandler(event) {
        ...
        if (pathName == 'disclaimer') {
            ...
            for (let i = 0; i < this.attendeeList.length; i++) {
                if (this.attendeeList[i].attendeefirstname == '' || this.attendeeList[i].attendeelastname == '' || this.attendeeList[i].attendeegender == '' || this.attendeeList[i].attendeeage == '' || (this.attendeeList[i].isRadioChecked && (this.attendeeList[i].attendeeemail == '' || this.attendeeList[i].attendeephone == ''))) {
                    isAttendeesValid = false;
                }
            }
    ...
    pathElementClasses(pathNumber) {
        ...
        for (var i = 0; i < pathElements.length; i++) {
            ...
            } else if (i > pathNumber) { //added by trupti
                console.log('pathNumber----'+pathNumber)
                console.log('receiptStepCompleted----'+receiptStepCompleted)
                if (receiptStepCompleted) {
                    pathElements[i].classList.remove('slds-is-active');
                    pathElements[i].classList.remove('slds-is-current');
                    pathElements[i].classList.remove('slds-is-complete');
                    pathElements[i].classList.add('disabled-link');  // Disable other path link
